        ...
        <DropZoneStyle
          ...
          <Box sx={{ p: 2, ml: { md: 2 } }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              {caption ?? <FormattedMessage defaultMessage="Drop files here or click to browse." />}
            </Typography>
          </Box>
        </DropZoneStyle>
  ...
  return (
    <StyledListItem {...otherProps} component={m.div} {...varFade().in}>
      ...
      <ListItemText
        ...
        secondary={
          status === 'uploading' ? (
            <LinearProgress variant="determinate" value={percent} sx={{ my: 1 }} />
          ) : (
            formatData(size)
          )
        }
      ...
      <ListItemSecondaryAction>{getStatus(status)}</ListItemSecondaryAction>
    </StyledListItem>
  );
};
...
const StatusIcon = styled(({ color, ...props }) => <Icon {...props} />)(({ theme, color }) => ({
  width: 20,
  height: 20,
