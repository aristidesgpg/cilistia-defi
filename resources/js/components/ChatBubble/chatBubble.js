...
const ChatBubble = ({ message, downloadFile }) => {
  ...
  const content = useMemo(() => {
    switch (message.type) {
      case 'attachment':
        return (
          <Attachment direction="row" isSender={isSender} alignItems="center" spacing={1}>
            ...
            <IconButton color="inherit" onClick={downloadFile} size="small">
  ...
  return (
    <Stack direction="row">
      <Stack direction="row" sx={{ ...(isSender && { ml: 'auto' }) }} spacing={2}>
        {!isSender && <ChatAvatar participant={participant} showPresence={false} />}
...
const Content = styled(({ isSender, ...otherProps }) => {
  ...
  backgroundColor: !isSender ? theme.palette.background.neutral : theme.palette.primary.lighter
}));
