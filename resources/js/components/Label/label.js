...
const Label = ({ children, color = 'default', variant = 'ghost', startIcon, endIcon, sx }) => {
  return (
    <Container
      color={color}
      variant={variant}
      ...
      {isValidElement(startIcon) && <IconWrapper sx={{ mr: 0.75 }}>{startIcon}</IconWrapper>}

      {children}

      {isValidElement(endIcon) && <IconWrapper sx={{ ml: 0.75 }}>{endIcon}</IconWrapper>}
    </Container>
  );
};
