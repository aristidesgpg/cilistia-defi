...
const Sidebar = ({ isOpenSidebar, onCloseSidebar, links, action, extras }) => {
  ...
  const { isCollapse, collapseClick, collapseHover, onHoverEnter, onHoverLeave } = useNavbarDrawer();

  useSidebarAutoClose(isOpenSidebar, onCloseSidebar);

  return (
    <SidebarContainer isCollapse={isCollapse} collapseClick={collapseClick} isDesktop={isDesktop}>
      {isDesktop ? (
        <Drawer
          open={true}
...
const SidebarContainer = styled('div', {
  ...
  [theme.breakpoints.up('lg')]: {
    width: isCollapse ? NAVBAR_CONFIG.DASHBOARD_COLLAPSE_WIDTH : NAVBAR_CONFIG.DASHBOARD_WIDTH,
    transition: theme.transitions.create('width', {
