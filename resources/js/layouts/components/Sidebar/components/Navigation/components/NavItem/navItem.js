...
const NavItem = ({ item, level, active, onToggle, open = false }) => {
  ...
  const renderContent = (
    <React.Fragment>
      ...
      <StyledListItemText disableTypography isCollapse={isCollapse} primary={item.title} />

      {!isCollapse && isFunction(onToggle) && (
        <Iconify icon={open ? arrowIosDownwardFill : arrowIosForwardFill} sx={{ width: 16, height: 16, ml: 1 }} />
      )}
    </React.Fragment>
  );
  ...
  return isExternal(item.path) ? (
    ...
    <StyledListItem onClick={onToggle} subItem={isSubItem} disabled={item.disabled} active={active}>
      {renderContent}
    </StyledListItem>
  ) : (
    <StyledListItem subItem={isSubItem} component={RouterLink} to={item.path} disabled={item.disabled} active={active}>
      {renderContent}
    </StyledListItem>
  );
};
