    ...
    <Card>
      <Spin spinning={loading}>
        <StyledCardContent>
          ...
          <StyledBox>
            <Typography user={user} component={ProfileLink} variant="subtitle2" sx={{ display: 'block' }} noWrap>
              {user.name}
            </Typography>
            ...
            <Stack direction="row" alignItems="center" sx={{ minWidth: 0 }}>
              ...
              <Typography variant="body2" color="text.secondary" noWrap>
                {!user.country ? <FormattedMessage defaultMessage="Unavailable" /> : <span>{user.country_name}</span>}
              </Typography>
            </Stack>
          </StyledBox>
...
const FollowButton = ({ user, fetchUser }) => {
  ...
  if (isEmpty(user.following)) {
    return (
      <LoadingButton color="inherit" variant="outlined" size="small" onClick={follow} loading={loading}>
...
export const UserLoader = forwardRef((props, ref) => {
  ...
  return [...Array(count)].map((_, index) => {
    return (
      <Grid key={index} ref={index !== 0 ? undefined : ref} {...{ xs: 12, sm: 6, md: 4 }} item>
        <Skeleton height={96} variant="rectangular" width="100%" sx={{ borderRadius: 2 }} animation="wave" />
      </Grid>
    );
  });
});
