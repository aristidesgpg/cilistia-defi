...
const ActionBar = () => {
    ...
    return (
        <ActionToolbar>
            ...
            <Button variant="contained" onClick={addCountry} sx={{borderRadius: '5px'}}>
                <AddIcon />
            </Button>
        </ActionToolbar>
    );
...
const CreateForm = ({closeModal}) => {
    ...
    const fetchCountries = useCallback(() => {
        request
            .get(route("admin.operating-country.available"))
    ...
    const submitForm = useCallback(
        (values) => {
            formRequest
                .post(route("admin.operating-country.create"), values)
                .then(() => {
